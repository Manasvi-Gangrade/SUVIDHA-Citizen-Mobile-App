...
const DISPUTE_CHART_DATA = [
  { month: "May", Billed: 320, Expected: 180 },
  { month: "Jun", Billed: 340, Expected: 200 },
  { month: "Jul", Billed: 410, Expected: 220 },
  { month: "Aug", Billed: 480, Expected: 230 },
  { month: "Sep", Billed: 510, Expected: 210 },
  { month: "Oct (Disputed)", Billed: 850, Expected: 240 },
];
...
export default function ServiceForm() {
  ...
  const [complaintCategory, setComplaintCategory] = useState("Incorrect Bill");
  const [, setLocation] = useLocation();

  useEffect(() => {
    setRefNo(`SVD-${Math.floor(1000 + Math.random() * 9000)}`);
  }, []);
    ...
    if (step < 3) {
      ...
      if (isOffline) {
        const offId = addOfflineRequest({
          ...
          dept: deptLabel,
          title: config.title,
          description: `Filed via Mobile - ${config.title}`,
        });
        setRefNo(offId);
      } else {
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
              ...
              {config.step2Fields.map(f => (
                <div key={f.label}>
                  ...
                  {f.type === "select" && (
                    <select
                      className="w-full h-11 px-3 border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-suvidha-navy/30 bg-white"
                      value={f.label === "Complaint Category" ? complaintCategory : undefined}
                      onChange={f.label === "Complaint Category" ? (e) => setComplaintCategory(e.target.value) : undefined}
                      data-testid={`select-${f.label.toLowerCase().replace(/\s+/g, "-")}`}
                    >
              ...
              {type === "complaint" && complaintCategory === "Incorrect Bill" && (
                <div className="bg-white rounded-2xl border border-border p-4 space-y-3 shadow-sm mt-3">
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Consumption Disparity Analysis</span>
                    <span className="text-[10px] text-muted-foreground">Disputed billing period: Oct (850 units vs 240 units expected)</span>
                  </div>
                  <div className="h-44 w-full text-xs">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={DISPUTE_CHART_DATA} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        <XAxis dataKey="month" stroke="#888888" fontSize={9} tickLine={false} axisLine={false} />
                        <YAxis stroke="#888888" fontSize={9} tickLine={false} axisLine={false} />
                        <Tooltip />
                        <Legend wrapperStyle={{ fontSize: 9 }} />
                        <Bar dataKey="Expected" fill="#1e3a8a" name="Expected (KWh)" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="Billed" fill="#f97316" name="Billed (KWh)" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                  <div className="bg-orange-50 border border-orange-200 text-orange-800 text-[10px] leading-normal p-2.5 rounded-xl flex gap-2">
                    <span className="font-bold">Note:</span>
                    <span>An abnormal spike of 254% is detected for the current month relative to your historical winter mean.</span>
                  </div>
                </div>
              )}
